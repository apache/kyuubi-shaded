...
RELEASE_VERSION=${RELEASE_VERSION:?"RELEASE_VERSION is required, e.g. 0.1.0"}
...
RELEASE_VERSION - Release version, must match pom.xml and not be SNAPSHOT (e.g. 0.1.0)
...
upload_svn_staging() {
  ...
  SRC_TGZ_FILE="apache-kyuubi-shaded-${RELEASE_VERSION}-source.tgz"

  echo "Copying release tarballs"
  cp "${RELEASE_DIR}/${SRC_TGZ_FILE}"        "${SVN_STAGING_DIR}/kyuubi-shaded-${RELEASE_TAG}/${SRC_TGZ_FILE}"
  cp "${RELEASE_DIR}/${SRC_TGZ_FILE}.asc"    "${SVN_STAGING_DIR}/kyuubi-shaded-${RELEASE_TAG}/${SRC_TGZ_FILE}.asc"
  cp "${RELEASE_DIR}/${SRC_TGZ_FILE}.sha512" "${SVN_STAGING_DIR}/kyuubi-shaded-${RELEASE_TAG}/${SRC_TGZ_FILE}.sha512"

  svn add "${SVN_STAGING_DIR}/kyuubi-shaded-${RELEASE_TAG}"

  echo "Uploading release tarballs to ${SVN_STAGING_REPO}/${RELEASE_TAG}"
  (
    cd "${SVN_STAGING_DIR}" && \
    svn commit --username "${ASF_USERNAME}" --password "${ASF_PASSWORD}" --message "Apache Kyuubi Shaded ${RELEASE_TAG}"
  )
  echo "Kyuubi Shaded tarballs uploaded"
}

upload_nexus_staging() {
  ${KYUUBI_DIR}/build/mvn clean deploy -DskipTests -Papache-release \
...
finalize_svn() {
  ...
  svn mv --username "${ASF_USERNAME}" --password "${ASF_PASSWORD}" --no-auth-cache \
     --message "Apache Kyuubi Shaded ${RELEASE_VERSION}" \
     "${SVN_STAGING_REPO}/kyuubi-shaded-${RELEASE_TAG}" "${SVN_RELEASE_REPO}/kyuubi-shaded-${RELEASE_VERSION}"
  echo "Kyuubi Shaded tarballs moved"
}
